...
toReportForLs source err =
  case err of
    ...
    AmbiguousVar region maybePrefix name h hs ->
      ambiguousNameForLs region maybePrefix name h hs "variable"

    AmbiguousType region maybePrefix name h hs ->
      ambiguousNameForLs region maybePrefix name h hs "type"

    AmbiguousVariant region maybePrefix name h hs ->
      ambiguousNameForLs region maybePrefix name h hs "variant"

    AmbiguousBinop region name h hs ->
      ambiguousNameForLs region Nothing name h hs "operator"
    ...
    NotFoundVar region prefix name possibleNames ->
      notFoundForLs region prefix name "variable" possibleNames

    NotFoundType region prefix name possibleNames ->
      notFoundForLs region prefix name "type" possibleNames

    NotFoundVariant region prefix name possibleNames ->
      notFoundForLs region prefix name "variant" possibleNames
    ...
    RecursiveAlias region name args tipe others ->
        aliasRecursionReportForLs region name args tipe others
    ...
    Shadowing name r1 r2 ->
      Report.Report "SHADOWING" r2 [] $
        Code.toSnippet source r1 Nothing
          ( D.reflow $ "The name `" <> Name.toChars name <> "` is first defined here:"
          , D.reflow $ "But then it is defined AGAIN over here."
          )
    ...
    TypeVarsUnboundInUnion unionRegion typeName allVars unbound unbounds ->
      unboundTypeVarsForLs unionRegion ["type"] typeName allVars unbound unbounds
      ...
      case (unusedVars, unboundVars) of
        (unused:unuseds, []) ->
          let
            ...
            (title, _subRegion, overview, stuff) =
              case unuseds of
                [] ->
                  ("UNUSED TYPE VARIABLE"
                  , Just (snd unused)
        ...
        ([], unbound:unbounds) ->
          unboundTypeVarsForLs aliasRegion ["type","alias"] typeName allVars unbound unbounds
...
-- BAD TYPE VARIABLES FOR LANGUAGE SERVER


unboundTypeVarsForLs :: A.Region -> [D.Doc] -> Name.Name -> [Name.Name] -> (Name.Name, A.Region) -> [(Name.Name, A.Region)] -> Report.Report
unboundTypeVarsForLs declRegion tipe typeName _allVars (unboundVar, varRegion) unboundVars =
  let
    backQuote name =
      "`" <> D.fromName name <> "`"
  ...
  in
  Report.Report title (Maybe.fromMaybe declRegion subRegion) [] $
    D.fillSep overview



-- AMBIGUOUS NAME FOR LANGUAGE SERVER


ambiguousNameForLs :: A.Region -> Maybe Name.Name -> Name.Name -> ModuleName.Canonical -> OneOrMore.OneOrMore ModuleName.Canonical -> String -> Report.Report
ambiguousNameForLs region maybePrefix name h hs thing =
  let
    possibleHomes = List.sort (h : OneOrMore.destruct (:) hs)
  in
  Report.Report "AMBIGUOUS NAME" region [] $
...
-- NOT FOUND FOR LANGUAGE SERVER


notFoundForLs :: A.Region -> Maybe Name.Name -> Name.Name -> String -> PossibleNames -> Report.Report
notFoundForLs region maybePrefix name thing (PossibleNames locals quals) =
  let
    givenName =
      maybe Name.toChars toQualString maybePrefix name
...
-- BAD ALIAS RECURSION FOR LANGUAGE SERVER


aliasRecursionReportForLs :: A.Region -> Name.Name -> [Name.Name] -> Src.Type -> [Name.Name] -> Report.Report
aliasRecursionReportForLs region name args tipe others =
  case others of
    [] ->
      Report.Report "ALIAS PROBLEM" region [] $
