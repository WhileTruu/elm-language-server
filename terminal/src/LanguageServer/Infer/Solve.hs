...
module LanguageServer.Infer.Solve
  ( runWithRegions
  )
  where
...
import LanguageServer.Infer.Type as Type
...
instance Ord A.Position where
  compare (A.Position r1 c1) (A.Position r2 c2) =
    compare (r1, c1) (r2, c2)


instance Ord A.Region where
  compare (A.Region s1 e1) (A.Region s2 e2) =
    compare (s1, e1) (s2, e2)
...
runWithRegions
  ...
  -> IO
       (Either
          (NE.List Error.Error)
          (Map.Map Name.Name Can.Annotation, Map.Map A.Region HoverInfo))
runWithRegions constraint =
  do  pools <- MVector.replicate 8 []

      (State env _ errors regionVars) <-
      ...
      case errors of
        [] ->
          do  annotations <- traverse Type.toAnnotation env
              regionTypes <-
                traverse
                  (\(maybeName, hoverKind, var) ->
                    do  Can.Forall _ tipe <- Type.toAnnotation var
                        return (HoverInfo maybeName hoverKind tipe)
                  )
                  regionVars
              return $ Right (annotations, regionTypes)
...
data State =
  State
    ...
    , _mark :: Mark
    , _errors :: [Error.Error]
    , _regionTypes :: Map.Map A.Region (Maybe Name.Name, HoverKind, Variable)
    }
...
solve env rank pools state constraint =
  case constraint of
    ...
    CSaveTheType region hoverKind maybeName tipe ->
      do  variable <- typeToVariable rank pools tipe
          return $
            state
              { _regionTypes =
                  Map.insertWith (\_ old -> old) region (maybeName, hoverKind, variable) (_regionTypes state)
              }
    ...
    CLocal region name expectation ->
      do  sourceVar <-
          ...
          actual <- makeCopy rank pools sourceVar
          expected <- expectedToVariable rank pools expectation
          answer <- Unify.unify actual expected
          case answer of
            Unify.Ok vars ->
    ...
    CLet rigids flexs header headerCon subCon ->
      do
          ...
          (State savedEnv mark errors regionTypes) <-
          ...
          let newEnv = Map.union env (Map.map A.toValue locals)
          let tempState = State savedEnv finalMark errors regionTypes
...
addError (State savedEnv rank errors regionTypes) err =
  State savedEnv rank (err:errors) regionTypes
