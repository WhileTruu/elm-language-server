{-# OPTIONS_GHC -Wall #-}
module LanguageServer.Infer.Expression
  ( constrain
  , constrainDef
...
import qualified LanguageServer.Infer.Pattern as Pattern
import qualified LanguageServer.Infer.Type as InferType
import LanguageServer.Infer.Type as Type hiding (Descriptor(..))
...
constrain rtv (A.At region expression) expected =
  do  exprCon <-
      ...
      return $
        CAnd
          [ exprCon
          , saveHoverType region expression expected
          ]
...
saveHoverType :: A.Region -> Can.Expr_ -> Expected Type -> Constraint
saveHoverType region expression expected =
  let tipe = expectedType expected in
  case expression of
    Can.VarLocal name ->
      InferType.CSaveTheType region InferType.HoverLocalValue (Just name) tipe

    Can.VarTopLevel _ name ->
      InferType.CSaveTheType region InferType.HoverTopLevelValue (Just name) tipe

    Can.VarForeign _ name _ ->
      InferType.CSaveTheType region InferType.HoverImportedValue (Just name) tipe

    Can.VarCtor _ _ name _ _ ->
      InferType.CSaveTheType region InferType.HoverConstructor (Just name) tipe

    Can.VarDebug _ name _ ->
      InferType.CSaveTheType region InferType.HoverImportedValue (Just name) tipe

    Can.VarOperator name _ _ _ ->
      InferType.CSaveTheType region InferType.HoverOperator (Just name) tipe

    Can.Accessor field ->
      InferType.CSaveTheType region InferType.HoverFieldAccessor (Just field) tipe

    _ ->
      InferType.CSaveTheType region InferType.HoverExpression Nothing tipe
    ...
    pattern : otherArgs ->
      do  argVar <- mkFlexVar
          ...
          (Args vars tipe result newState) <-
            argsHelp otherArgs =<<
              Pattern.addWithKind InferType.HoverLocalParameter pattern (PNoExpectation argType) state
      ...
      do  argType <- Instantiate.fromSrcType rtv srcType
          ...
          (TypedArgs tipe resultType newState) <-
            typedArgsHelp rtv name (Index.next index) otherArgs srcResultType =<<
              Pattern.addWithKind InferType.HoverLocalParameter pattern expected state
